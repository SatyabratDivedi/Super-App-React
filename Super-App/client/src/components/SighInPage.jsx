...
const SighInPage = ({signInFormRef, setIsSighIn, signUpFormRef}) => {
    ...
    const errorRefs = Array(2).fill().map(() => useRef());
    const borderRefs = Array(2).fill().map(() => useRef());
            ...
            <div className='w-[50%] text-center'>
                ...
                <div>
                    <form onSubmit={sighIntHandle} className='form'>
                        ...
                        <button type='submit' className='bg-[#72DB73] rounded-2xl my-3 font-roboto font-bold p-[.2rem] active:scale-95 duration-200'>Sing In</button>
                    </form>
                </div>
                <div className=' text-start border-b borderw w-[60%] mt-3'> Not an account <span onClick={signUpSlideHandle} className=' font-roboto text-[#72DB73] cursor-pointer'>Sign Up</span> </div>
            </div>
